...
import {  viewRescheduledForTechnicalUrl } from '../../../api/constants'
//import CheckBox from './CheckBox'
import ClearIcon from '@mui/icons-material/Clear';

import SearchAppBar from './TechnicalHeader'
...
import TooltipSendNextTechnical from './TooltipSendNextTechnical'
...
import { deleteFromRescheduleHandler, postHrScheduleInterview, rejectedListUrlHandler,doNotHireUrlHandler } from '../../../api/api'
...
const CustomerData = (props) => {
  ...
  <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
    ...
    <TableCell > <TooltipSendNextTechnical props={row}/></TableCell> 
...
const NextTechnicalRound= (props) => {
...
const allRejectHandler = () =>
{
  ...
  useEffect (() =>{
    const fetchData=async () =>{
      const response = await fetch(viewRescheduledForTechnicalUrl);
...
const deleteCandidateByIds = () => {
    ...
    try{
      deleteFromRescheduleHandler(arrayids,token)
      setIsDeleted(true)
    }
    catch(error){
      setHttpError(error.message)
...
return (
    ...
    <div>
      ...
      <Tooltip title="Shorlisted For Management Round">
      ...
      <Table id="emp-table" aria-label="collapsible table">
        ...
        <TableBody>
          {candidateData.filter((e) => { return e.firstName?.toLowerCase().indexOf(search) >= 0 || e.lastName?.toLowerCase().indexOf(search) >= 0 || e.emailid?.toLowerCase().indexOf(search) >= 0 || e.jobRole?.toLowerCase().indexOf(search) >= 0 }).sort((a, b) => a.firstName.localeCompare(b.firstName)).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).sort((a, b) => a.firstName.localeCompare(b.firstName)).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(row => (
...
export default NextTechnicalRound