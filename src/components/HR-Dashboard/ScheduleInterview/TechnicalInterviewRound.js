...
import { viewInterviewCandidateUrl } from '../../../api/constants'
//import CheckBox from './CheckBox'
 
import SearchAppBar from './ScheduleInterviewHeader'
import TablePagination from '@mui/material/TablePagination';
import Modal from '../../ui/Modal'
import TooltipForSendInterview from './TooltipForSendInterview'
...
import classes from './ScheduleInterviewList.module.css'

import { deleteFromTechnicalTest,rejectedListUrlHandler, doNotHireUrlHandler, postHrScheduleInterview, postRescheduleTypeHandler } from '../../../api/api'
...
const CustomerData = (props) => {
  ...
  <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
    ...
    <TableCell > <TooltipForSendInterview props={row}/></TableCell> 
  ...
  const [isNextShortlist, setIsNextShortlist] = React.useState();
  ...
  const [isDoNotHire, setIsDoNotHire] = useState();
  ...
  const allDoNotHireHandler =() => {
    setIsDoNotHire(false)
  }
...
setIsRejected(false)
  ...
  useEffect (() =>{
    const fetchData=async () =>{
      const response = await fetch(viewInterviewCandidateUrl);
  ...
  const deleteCandidateByIds = () => {
    ...
    try{
      deleteFromTechnicalTest(arrayids,token)
      setIsDeleted(true)
    }
    catch(error){
      setHttpError(error.message)
...
const shortlistCandidateByHR = () => {
  ...
  try{
    postRescheduleTypeHandler(arrayids,token)
    setIsNextShortlist(true)
  }
  catch(error){
    setHttpError(error.message)
    setIsNextShortlist(false)
...
const doNotHireHandler = () => {
  ...
  try{
    doNotHireUrlHandler(arrayids,token)
    setIsDoNotHire(true)
  }
  catch(error){
    setHttpError(error.message)
...
const shortlistCandidateForHR = () => {
  ...
  try{
    postHrScheduleInterview(arrayids,token)
...
//   }
  ...
  return (
    ...
    <SearchAppBar onChange={(event) => { setSearch(event.target.value.toLowerCase()) }}/>
      ...
      <Tooltip title="Shortlisted For Next Technical Interview">
        <Fab size="small" color="primary" aria-label="add" sx={{ mt: 1, mb: 1, ml: 2 }} onClick={() => shortlistCandidateByHR()}>
          <AddIcon />
        </Fab>
      </Tooltip>
      {
                 isNextShortlist && <Modal onClose={props.onClose}>
            <Box sx={{
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'space-between',
              ...
              Shorlist successfully!
              <Button
                onClick={allDetailsShowHandler}
                variant="contained"
                sx={{
                  mt: 1, mb: 1, mr: 3, fontSize: '0.8rem', background: '#FFB319', '&:hover': {
                    background: "#FFB344",
      ...
      <Tooltip title="Shorlisted For Mangement Round">
        <Fab size="small" color="primary" aria-label="add" sx={{ mt: 1, mb: 1, ml: 2 }} onClick={() => shortlistCandidateForHR()}>
          <AddBoxOutlinedIcon/>
        </Fab>
      </Tooltip>
      {
            ...
            <Box sx={{
              ...
              Shorlist successfully!
              <Button
                onClick={allDetailsCloseHandler}
                variant="contained"
                sx={{
                  mt: 1, mb: 1, mr: 3, fontSize: '0.8rem', background: '#FFB319', '&:hover': {
                    background: "#FFB344",
      ...
      <Tooltip title="Reject">
        <Fab size="small" color="primary" aria-label="add" sx={{ mt: 1, mb: 1, ml: 2 }}  onClick={()=> rejectCandidateByHR()}>
          <DoNotDisturbOffIcon />
        </Fab>
      </Tooltip>
      {
      ...
      <Tooltip title="Do Not Hire">
        <Fab  size="small" color="primary" aria-label="add" sx={{ mt: 1, mb: 1, ml:2 }}  onClick ={()=> doNotHireHandler()}>
          <ClearIcon />
        </Fab>
      </Tooltip>

      {
                 isDoNotHire && <Modal onClose={props.onClose}>
            <Box sx={{
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'space-between',
              ...
              Do Not Hire successfully!
              <Button
                onClick={allDoNotHireHandler}
                variant="contained"
                sx={{
                  mt: 1, mb: 1, mr: 3, fontSize: '0.8rem', background: '#FFB319', '&:hover': {
                    background: "#FFB344",
