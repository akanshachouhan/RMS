...
import { secondTechnicalRoundHandler} from '../../../api/api.js';
...
const TooltipShortlist = (props) => {
    ...
    const[formManager, setFormManager] = React.useState({interviewLink: '', carbonCopy:''
  })
...
//   const userEmail = JSON.parse(localStorage.getItem('userData')).emailid;
    ...
    const sendEmail = (event) => {
      ...
      emailid: props.props.emailid,
          ...
          carbonCopy : data.get('carbonCopy')
        }
        console.log(scheduleTestData)
        
        secondTechnicalRoundHandler(scheduleTestData)
        setAddedJobs(true)
      }
    ...
    return (
        <div>
            <Tooltip title="Schedule Next Technical Interview">
          ...
          {/* <Box>
            ...
            <Box sx={{ textAlign: 'center', m: 1, fontWeight: 'bold' }}>Send Technical Interview Link to {props.props.firstName} {props.props.lastName}</Box>
            ...
            <TextareaAutosize id="filled-basic" aria-label="minimum height"
                ...
                onChange={(event) => {managerHandler(event,'interviewLink')}}
                style={{ width: 400 }} required fullWidth name="interviewLink" label="Message with test link" variant="filled" />

<TextareaAutosize id="filled-basic" aria-label="minimum height"
                minRows={2}
                placeholder="Enter EmailId"
                onChange={(event) => {managerHandler(event,'carbonCopy')}}
                style={{ width: 400 }} required fullWidth name="carbonCopy" label="EmailIds Of Tech Leads" variant="filled" />
