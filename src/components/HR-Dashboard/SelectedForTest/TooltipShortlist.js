...
import { testLinkMailToCandidate } from '../../../api/api.js';
...
//   const userEmail = JSON.parse(localStorage.getItem('userData')).emailid;
    ...
    const sendEmail = (event) => {
      ...
      emailid: props.props.emailid,
        ...
        testLinkMailToCandidate(scheduleTestData)
        setAddedJobs(true)
      }
    ...
    return (
        <div>
          ...
          {/* <Box>
            ...
            <Box sx={{ textAlign: 'center', m: 1, fontWeight: 'bold' }}>Send test email to {props.props.firstName} {props.props.lastName}</Box>
            ...
            <TextareaAutosize id="filled-basic" aria-label="minimum height"
                ...
                onChange={(event) => {managerHandler(event,'testLink')}}
                style={{ width: 400 }} required fullWidth name="testLink" label="Message with test link" variant="filled" />
