...
import { shortlistedByManagerListUrl } from '../../../api/constants'

import classes from './ShortlistedByManagerList.module.css'
import SearchAppBar from './ShortlistedByManagerHeader'
import { CircularProgress } from '@mui/material'
import TablePagination from '@mui/material/TablePagination';
import { sendDataInFinalListHandler,deleteFromShortlistHanlder } from '../../../api/api'
...
const CustomerData = (props) => {
  ...
  <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
    ...
    <TableCell > <TooltipComment props={row}/></TableCell> 
  ...
  useEffect (() =>{
    const fetchData=async () =>{
      const response = await fetch(shortlistedByManagerListUrl);
...
const deleteCandidateByIds = () => {
    ...
    try{
      deleteFromShortlistHanlder(arrayids)
      setIsDeleted(true)
    }
    catch(error){
      setHttpError(error.message)
...
const sendCandidateToFinalList = () => {
    ...
    try {
      sendDataInFinalListHandler(arrayids,token)
