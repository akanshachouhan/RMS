...
import {  viewDoNotHireUrl } from '../../../api/constants'
...
import { deleteFromDoNotHireHanlder } from '../../../api/api'
 ...
 ;
  ...
  useEffect (() =>{
    const fetchData=async () =>{
      const response = await fetch(viewDoNotHireUrl);
...
const deleteCandidateByIds = () => {
    ...
    try{
  deleteFromDoNotHireHanlder(arrayids,token)
      setIsDeleted(true)
    }
    catch(error){
      setHttpError(error.message)
