...
import {  viewRejectListUrl } from '../../../api/constants'
import Modal from '../../ui/Modal'
import { Button } from '@mui/material';

// import classes from './CandidateList.module.css'
import SearchAppBar from './RejectHeader'

import classes from './DoNotHireList.module.css'
...
import TooltipRejection from './TooltipRejection'
...
import { deleteFromRejectHanlder } from '../../../api/api'
...
const CustomerData = (props) => {
  ...
  <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
    ...
    <TableCell><TooltipRejection props={row}/></TableCell>
  ...
  useEffect (() =>{
    const fetchData=async () =>{
      const response = await fetch(viewRejectListUrl);
...
const deleteCandidateByIds = () => {
    ...
    try{
  deleteFromRejectHanlder(arrayids,token)
      setIsDeleted(true)
    }
    catch(error){
      setHttpError(error.message)
...
// const sendMailtoReport = () => {
//   let arrayids = [];
//   // console.log(deleteCandidateByIds)
//   candidateData.forEach(d => {
//       if (d.select) {
//           arrayids.push(d.emailid);
//       }
